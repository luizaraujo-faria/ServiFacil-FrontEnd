...
function Loggin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      if (formData.salvarAcesso) {
        localStorage.setItem('token', result.user.token); // salva token se checkbox marcado
      }
      ...
      <div className="flex flex-1 items-center justify-center bg-white p-6 md:p-8 md:w-1/2">
        <div className="w-full max-w-md bg-white p-6 md:p-8 rounded-xl shadow-lg">
          ...
          <p className="mt-2 text-center text-gray-600">
            É um profissional?{' '}
            <Link to="/cadastroProfissional" className="text-yellow-500 hover:underline">
              Cadastre-se aqui
            </Link>
          </p>
        </div>
